...
def convert_edge_index_to_edge_hash(edge_index, num_nodes=None):
    ...
    edge_index = convert_to_jax(edge_index, dtype=jnp.int64)
...
def convert_edge_hash_to_edge_index(edge_hash, num_nodes):
    ...
    edge_hash = convert_to_jax(edge_hash, dtype=jnp.int64)

    # num_nodes = tf.cast(num_nodes, tf.int64)

    row = jnp.floor_divide(edge_hash, num_nodes)
    col = jnp.mod(edge_hash, num_nodes)
...
def merge_duplicated_edge(edge_index, edge_props=None, merge_modes=None):
    ...
    edge_index = convert_to_jax(edge_index)
    ...
    unique_edge_hash, unique_index = jnp.unique(edge_hash, return_inverse=True)
    ...
    if edge_props is None:
        ...
        for edge_prop, merge_mode in zip(edge_props, merge_modes):
            ...
            if edge_prop is None:
                ...
                edge_prop = convert_to_jax(edge_prop)
                ...
                unique_edge_prop = merge_func(edge_prop, unique_index, unique_edge_hash.shape[0])
...
def convert_edge_to_upper(edge_index, edge_props=None, merge_modes=None):
    ...
    row = jnp.min(edge_index, axis=0)
    col = jnp.max(edge_index, axis=0)

    upper_edge_index = jnp.stack([row, col], axis=0)
...
def convert_edge_to_directed(edge_index, edge_props=None, merge_modes=None):
    ...
    non_self_loop_mask = jnp.not_equal(upper_edge_index[0], upper_edge_index[1])

    if jnp.any(non_self_loop_mask):
        non_self_loop_edge_index = upper_edge_index[:, non_self_loop_mask]
        pure_lower_edge_index = jnp.stack([
            non_self_loop_edge_index[1],
            non_self_loop_edge_index[0]
        ], axis=0)
        updated_edge_index = jnp.concatenate([upper_edge_index, pure_lower_edge_index], axis=1)
        ...
        if edge_props is None:
            ...
            for edge_prop, upper_edge_prop, merge_mode in zip(edge_props, upper_edge_props, merge_modes):
                if edge_prop is None:
                    ...
                    pure_lower_edge_prop = upper_edge_prop[non_self_loop_mask]
                    updated_edge_prop = jnp.concatenate([upper_edge_prop, pure_lower_edge_prop], axis=0)
...
def remove_self_loop_edge(edge_index, edge_weight=None):
    ...
    edge_index = convert_to_jax(edge_index)
    edge_weight = convert_to_jax(edge_weight)

    row, col = edge_index[0], edge_index[1]
    mask = jnp.not_equal(row, col)
