...
from jax_gnn.utils.graph_utils import convert_edge_to_directed, compute_edge_mask_by_node_index
...
class Graph(object):
    ...
    def __init__(self, x, edge_index, y=None, edge_weight=None):
        ...
        if edge_weight is not None:
            ...
            if isinstance(self.edge_index, jnp.ndarray):
                self.edge_weight = jnp.ones([self.num_edges], dtype=jnp.float64)
            else:
                self.edge_weight = np.ones([self.num_edges], dtype=np.float64)
    ...
    def cast_edge_weight(cls, edge_weight):
        if isinstance(edge_weight, list):
            edge_weight = np.array(edge_weight).astype(np.float64)
    ...
    def adj(self):
        ...
        return jsp.SparseMatrix(self.edge_index, self.edge_weight, shape=[num_nodes, num_nodes])

    def to_jax(self):
        return Graph(
            convert_to_jax(self.x),
